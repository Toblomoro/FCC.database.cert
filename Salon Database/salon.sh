...
CREATE_APPOINTMENT(){
  ...
  echo -e "\nWhat time would you like your $SERVICE_NAME, $CUST_NAME_FORMATTED?"
  read SERVICE_TIME
  INSERTED=$($PSQL "INSERT INTO appointments (customer_id, service_id, time) VALUES ($CUST_ID, $SERVICE_ID_SELECTED, '$SERVICE_TIME')")
  echo -e "\nI have put you down for a $SERVICE_NAME at $SERVICE_TIME, $CUST_NAME_FORMATTED."
}
...
MAIN_MENU(){
  ...
  if [[ -z $HAVE_CUST ]]
  then
  echo -e "\nI dont have a record of that phone number, what is your name?"
  read CUSTOMER_NAME
...
# HAVE_PHONENUM=$($PSQL "SELECT customer_id, name FROM customers WHERE phone='$CUSTOMER_PHONE'")

# if
# [[ -z $HAVE_PHONENUM ]]
# then
# echo -e "\nI dont have any record of that phone number. What is your name?\n"
# read CUSTOMER_NAME
...
# read SERVICE_TIME
# else
# echo -e "Excellent, i have you booked for $SERVICE_NAME. What time would you like to book that?"
# fi
